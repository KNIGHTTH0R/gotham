    ...
    <div style="padding:0; background-color:#2c2c2c; padding:5px;" class="col-md-6 col-md-offset-3">
        <table style="width:100%">
            <th colspan="{{$colspan}}" style="padding:10px;border-bottom:3px solid #5f4a3d;background-color: #212121">
                Select user(s) to remove from: <br />{{$group->name}}</th>
            <form class="form-register" method="POST" action="/groups/remove_user">
            {{csrf_field()}}
            ...
            <?php 
                ...
                $groupUserCollection = \gotham\Group::find($group->id)->users()->get();
                
                //$diff = $enabledUserCollection->diff($groupUserCollection);
                $diff = $groupUserCollection;
            ...
            @else
                <tr><td style="text-align:center;padding-top:5px;font-weight:bolder; color:black" colspan="{{$colspan}}">No enabled users found that can be removed from Group</td></tr>
            @endif
